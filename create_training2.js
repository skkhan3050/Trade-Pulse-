...
// Extract the Razorpay script
let razorpayScript = '';
let razorpayStart = indexContent.indexOf('<!-- Razorpay Checkout Integration -->');
if (razorpayStart !== -1) {
  let razorpayEnd = indexContent.indexOf('</body>', razorpayStart);
  if (razorpayEnd !== -1) {
    razorpayScript = indexContent.substring(razorpayStart, razorpayEnd);
  }
}
...
// Now strip Razorpay from index.html
let newIndexContent = indexContent;
if (razorpayStart !== -1) {
  let razorpayEnd = indexContent.indexOf('</body>', razorpayStart);
  if (razorpayEnd !== -1) {
     newIndexContent = indexContent.substring(0, razorpayStart) + indexContent.substring(razorpayEnd);
  }
}

// Ensure the buttons on index.html link to training.php
newIndexContent = newIndexContent.replace(
  ...
  'href="training.php">Watch the Free Training'
);
...
newIndexContent = newIndexContent.replace(
  ...
  'href="training.php"$1>Watch the Free Training now.</a'
);

newIndexContent = newIndexContent.replace(
  /onclick="\s*document\.getElementById\('demoModal'\)\.classList\.add\('show'\)\s*"\s*>([\s\S]*?)Book Your\s*2-Day Workshop<\/a/g,
  'href="training.php">$1Watch the Free Training</a'
);
...
console.log('Fixed training.php and index.html');
